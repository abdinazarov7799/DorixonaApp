...
import {Badge, Box, Center, Flex, Heading, Icon} from "native-base";
...
const OutgoingOnSignTab = () => {
    const {t} = useTranslation()
    const router = useRouter();
    const {data, isRefreshing, onRefresh, onEndReached, isFetchingNextPage,isLoading} = useInfiniteScroll({
        key: `${KEYS.outgoing}`, url: ENDPOINTS.outgoing, limit: 15, filters: {
            include: 'titleShort,cardType,correspondentTitle,regNoInternalFull',
            "docStatus[]": 'on_sign',
    ...
    return (<>
        ...
        <FlatList data={data}
                      ...
                      renderItem={(item) => {
                          return <TouchableOpacity onPress={() => router.push(`/outgoing/${get(item, 'item.id')}`)}>
                              <Box
                              ...
                              >
                                  <Flex direction={'row'} alignItems={'center'} justifyContent={'space-between'}>
                                      ...
                                      <Text className={'text-xs'}>№{get(item, 'item.regNoInternalFull')} <Icon size={3}
...
const OutgoingTab = () => {
    ...
    const {t} = useTranslation()
    const {data, isRefreshing, onRefresh, onEndReached, isFetchingNextPage,isLoading} = useInfiniteScroll({
        key: `${KEYS.outgoing}-all`, url: ENDPOINTS.outgoing, limit: 15, filters: {
            include: 'titleShort,cardType,correspondentTitle,regNoInternalFull',
            "filter[is_deleted][neq]": 1,
            "sort": '-id',
        }
    ...
    return (<>
        ...
        refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={onRefresh}/>}
        renderItem={(item) => {
            return <TouchableOpacity onPress={() => router.push(`/outgoing/${get(item, 'item.id')}`)}><Box
                className={clsx('rounded mx-1 bg-white', {'!bg-[#fffbe6]': isEqual(get(item, 'item.cardType'), "3"), '!bg-[#fff2f0]': isEqual(get(item, 'item.cardType'), "2"), '!bg-[#e6f7ff]': isEqual(get(item, 'item.cardType'), "1")})}
                p={3} mt={3} shadow={2}
            >
                <Flex direction={'row'} alignItems={'center'} justifyContent={'space-between'}>
                    <DocflowStatus item={get(item, 'item', {})}/>
                    <Text className={'text-xs'}>№{get(item, 'item.regNoInternalFull')} <Icon size={3} as={MaterialCommunityIcons}
                                                                                  name="calendar"/> {dayjs(get(item, 'item.reg_datetime')).format("DD.MM.YYYY HH:MM")}
                    </Text>
                </Flex>
                <Heading size={'xs'} className={'my-2'}>{get(item, 'item.titleShort', '-')}</Heading>
...
export default function TabOutgoingScreen() {
    const {t} = useTranslation()
    return (<>
            ...
            <Center flex={1} px="3" className={"bg-[#F2F2F2] flex-1 flex"}>
                <CustomTabView routes={[{key: 'first', title: t('Not signed')}, {key: 'two', title: t('All signed')}]}
                               scenes={[{key: 'first', render: () => <OutgoingOnSignTab/>}, {
                                   key: 'two', render: () => <OutgoingTab/>
                               }]}/>
            </Center>
        </>
    );
